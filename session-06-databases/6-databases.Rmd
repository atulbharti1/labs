...
names(flights) # to check the variable names

joined_data <- dplyr::left_join(
  flights,
  planes |> dplyr::rename(year_built = year),
  by = "tailnum"
) # creating a new DF which has the merged df of flights and planes

setdiff(names(joined_data), names(flights)) #checking in the joined_data that which variables have come from the the df:planes
